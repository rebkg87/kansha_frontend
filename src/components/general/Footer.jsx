import React from 'react'

export const Footer = () => {
    return (
        <footer class="h-[309px] bg-soft-green flex flex-col justify-center items-center bottom-0 left-0 z-20 w-full p-4 ">
            <div className='flex flex-row justify-center items-center gap-11 mb-3'>
                <a href="https://www.instagram.com/" target="_blank" className="flex items-center justify-center" aria-label="Enlace a X">
                    <svg xmlns="http://www.w3.org/2000/svg" width="34" height="34" fill="none" viewBox="0 0 20 20">
                        <path className='fill-dark-green hover:fill-medium-soft-green transition-colors' d="M19.65 5.782c-.001 2.812.004 5.623-.001 8.436-.005 2.71-1.745 4.844-4.387 5.377-.35.07-.714.084-1.075.082l-8.294-.001c-2.736-.001-4.822-1.698-5.391-4.391-.077-.364-.089-.74-.089-1.112C.416 12.782.415 11.39.415 10c0-1.391-.002-2.782 0-4.174C.417 3.081 2.116.973 4.787.41A5.26 5.26 0 0 1 5.863.323c2.774 0 5.549-.002 8.324 0 2.723.004 4.834 1.73 5.38 4.406.07.345.083.7.083 1.053Zm-1.704 8.387c.005-2.782.004-5.562-.001-8.343-.001-.453-.043-.902-.2-1.335-.567-1.557-1.863-2.451-3.584-2.454a2932.54 2932.54 0 0 0-8.262.001c-.31 0-.622.013-.925.086-1.758.421-2.849 1.813-2.855 3.678-.01 2.8-.004 5.601 0 8.403 0 .694.132 1.36.5 1.96.746 1.216 1.846 1.794 3.255 1.798 1.381.004 2.763.001 4.146.001 1.372 0 2.744.003 4.115-.001.321-.001.643-.012.956-.087 1.768-.424 2.851-1.825 2.855-3.707Z" />
                        <path className='fill-dark-green  hover:fill-medium-soft-green transition-colors' d="M15.188 3.57a1.258 1.258 0 0 1 1.264 1.255 1.27 1.27 0 0 1-1.247 1.246 1.261 1.261 0 0 1-1.26-1.26 1.247 1.247 0 0 1 1.243-1.241Zm-5.161 1.447c2.728-.006 4.96 2.238 4.957 4.985-.001 2.75-2.236 4.992-4.962 4.98a4.976 4.976 0 0 1-4.943-5c.02-2.747 2.222-4.958 4.948-4.965Zm3.255 4.991c0-1.798-1.454-3.272-3.234-3.276a3.267 3.267 0 0 0-3.265 3.275c.003 1.795 1.465 3.261 3.25 3.261 1.783 0 3.246-1.468 3.248-3.26Z" />
                    </svg>
                </a>
                <a href="https://www.instagram.com/" target="_blank" className="flex items-center justify-center" aria-label="Enlace a X">
                    <svg xmlns="http://www.w3.org/2000/svg" width="34" height="34" fill="none" viewBox="0 0 22 20">
                        <g clip-path="url(#a)">
                            <path className='fill-dark-green hover:fill-medium-soft-green transition-colors' d="M20.074 19.677H.651V.323h19.423v19.354Zm-10.72-8.222c.017.02.028.03.037.042 1.269 1.696 2.537 3.39 3.803 5.088.065.088.13.123.24.123 1.283-.003 2.566-.002 3.848-.002h.187L11.656 8.99c1.77-1.897 3.531-3.781 5.32-5.697-.397 0-.743-.003-1.089.004-.052 0-.114.041-.152.082-.207.216-.408.438-.612.657a9740.609 9740.609 0 0 1-3.44 3.672l-.548.579c-.038-.047-.068-.08-.095-.116L7.47 3.429c-.06-.079-.113-.14-.234-.14-1.272.007-2.544.004-3.817.005-.042 0-.085.007-.152.012l5.566 7.443-5.577 5.957c.41 0 .771.002 1.132-.003.044 0 .096-.036.128-.07.741-.79 1.48-1.583 2.22-2.375l2.619-2.803Z" />
                            <path className='fill-soft-green' d="m9.354 11.455-2.62 2.802c-.74.792-1.477 1.585-2.219 2.376-.032.034-.084.069-.128.07-.36.005-.721.003-1.132.003l5.577-5.957C6.977 8.27 5.129 5.8 3.266 3.306c.067-.005.11-.012.152-.012 1.273 0 2.545.002 3.817-.004.121 0 .174.06.233.139l3.572 4.743c.027.035.057.07.095.116.188-.2.37-.388.548-.58l3.44-3.672c.204-.218.404-.44.612-.657.038-.04.1-.08.152-.081.346-.008.691-.004 1.09-.004L11.656 8.99l5.813 7.715h-.187c-1.282 0-2.565-.002-3.847.002a.268.268 0 0 1-.242-.123c-1.264-1.698-2.533-3.392-3.801-5.088l-.038-.042Zm-4.297-7.27.264.352 4.88 6.46c1.168 1.546 2.336 3.09 3.501 4.638a.266.266 0 0 0 .243.121c.527-.006 1.055-.003 1.582-.003h.165c-.037-.055-.051-.076-.067-.096l-4.122-5.484c-1.48-1.968-2.958-3.936-4.44-5.902A.24.24 0 0 0 6.9 4.19c-.243-.008-.486-.003-.73-.003H5.058Z" />
                            <path className='fill-dark-green hover:fill-medium-soft-green transition-colors' d="M5.057 4.186h1.115c.243 0 .486-.005.728.003a.24.24 0 0 1 .162.082c1.483 1.966 2.962 3.934 4.441 5.902l4.122 5.484c.015.02.03.041.067.096h-.165c-.527 0-1.055-.003-1.582.003a.266.266 0 0 1-.243-.121c-1.165-1.547-2.333-3.092-3.5-4.637L5.32 4.537l-.264-.351Z" />
                        </g>
                        <defs>
                            <clipPath id="a">
                                <path fill="#fff" d="M.651.323h20.968v19.355H.651z" />
                            </clipPath>
                        </defs>
                    </svg>
                </a>
                <a href="https://www.instagram.com/" target="_blank" className="flex items-center justify-center" aria-label="Enlace a X">
                    <svg xmlns="http://www.w3.org/2000/svg" width="34" height="34" fill="none" viewBox="0 0 22 20">
                        <g clip-path="url(#a)">
                            <path className='fill-dark-green hover:fill-medium-soft-green transition-colors' d="M10.32.323h7.769c1.233.001 1.934.695 1.91 1.939-.105 5.2-.024 10.4-.049 15.6-.003.616-.184 1.124-.7 1.485-.325.227-.69.323-1.08.324-1.308.003-2.616-.008-3.925.006-.354.004-.474-.119-.472-.476.012-2.252.014-4.505 0-6.757-.003-.4.137-.522.522-.509.695.023 1.39 0 2.085.004.347.001.524-.116.506-.507-.028-.6-.023-1.202 0-1.802.013-.351-.131-.445-.457-.438-.722.017-1.445-.011-2.167.013-.385.012-.505-.138-.486-.504.023-.422.018-.844.071-1.266.093-.73.536-1.225 1.257-1.366a5.007 5.007 0 0 1 1.343-.082c.31.023.448-.085.44-.415-.017-.64-.01-1.283-.003-1.924.002-.229-.07-.349-.321-.37-1.097-.09-2.178-.015-3.234.302-1.243.373-1.849 1.322-2.208 2.486-.252.817-.297 1.663-.324 2.509-.02.62-.008.62-.646.622-.34 0-.681.01-1.022-.003-.271-.01-.4.083-.394.375.013.669.013 1.338 0 2.006-.006.298.132.376.399.37.45-.012.9.004 1.349-.004.246-.005.326.09.325.341-.012 2.293-.02 4.587-.007 6.88.002.398-.135.516-.52.515-2.617-.012-5.234 0-7.85-.008-1.101-.003-1.803-.695-1.804-1.794-.01-5.27-.01-10.539-.005-15.808C.623 1.026 1.338.33 2.387.328 5.031.322 7.676.326 10.32.326V.323Z" />
                        </g>
                        <defs>
                            <clipPath id="a">
                                <path fill="#fff" d="M.619.323h20.968v19.355H.619z" />
                            </clipPath>
                        </defs>
                    </svg>

                </a>
            </div>
            <ul class="flex flex-col text-footer text-medium-green leading-footer gap-1 justify-center items-center mt-3 font-medium sm:mt-0">
                <li>
                    <a href="#" class="hover:underline text-medium-green">Política de Privacidad</a>
                </li>
                <li>
                    <a href="#" class="hover:underline text-medium-green">Términos de Servicios</a>
                </li>
                <li>
                    <a href="#" class="hover:underline text-medium-green">Ayuda</a>
                </li>
            </ul>
            <img className='h-[100px] w-[135px] mt-5' src='\assets\images\logo\logo-vert.png' />
        </footer>


    )
}
